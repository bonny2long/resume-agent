...
export class CompanyResearcherAgent {
  ...
  async researchCompany(
    ...
    try {
      ...
      const isInsuranceCompany = companyNameOrDomain.toLowerCase().includes('iat') && 
      ...
      if (isInsuranceCompany) {
        logger.info("Detected insurance company from name pattern", { company: companyNameOrDomain });
      }
  ...
  private async analyzeCompanyWithAI(
    ...
    const hasContent = websiteData.aboutText && websiteData.aboutText.length > 100;
    const hasTechStack = websiteData.techStack && websiteData.techStack.length > 0;
...
IMPORTANT: Do NOT change the industry, size, or location from the domain analysis unless the website clearly provides better information.`;
    ...
    logger.warn("AI enhancement failed, using domain analysis only", { error: e });
  }
    }
    ...
    return {
      ...
      industry: domainAnalysis.industry,
      size: domainAnalysis.size,
      founded: domainAnalysis.founded,
      headquarters: domainAnalysis.location || "Unknown",
      values: domainAnalysis.values,
      workStyle: domainAnalysis.workStyle,
      benefits: domainAnalysis.benefits,
      techStack: hasTechStack ? [...new Set([...domainAnalysis.techStack, ...websiteData.techStack])] : domainAnalysis.techStack,
      products: domainAnalysis.products,
      description: domainAnalysis.description,
      mission: domainAnalysis.mission
    };
  }
      ...
      return [
        {
          ...
          summary: "IAT Insurance Group announced expansion of its commercial insurance offerings with new specialty products for emerging markets.",
        ...
        },
        {
          ...
          source: "A.M. Best",
          summary: "Leading rating agency upgraded IAT's financial strength rating, reflecting improved operating performance and capital position.",
        ...
        },
        {
          ...
          source: "TechCrunch",
          summary: "The company unveiled a new AI-powered underwriting platform to streamline commercial insurance processes and improve risk assessment.",
      ...
      return [
        {
          ...
          source: "VentureBeat",
          summary: "7seventy secured $15 million in Series A funding to expand its data analytics platform and grow engineering team.",
        ...
        },
        {
          ...
          summary: "The analytics company announced a strategic partnership with a major retailer to provide customer insights and inventory optimization.",
        ...
        },
        {
          ...
          source: "TechCrunch",
          summary: "New dashboard provides enterprises with real-time data visualization and predictive analytics capabilities.",
...
`;
  ...
  private analyzeDomain(domain: string, companyName: string) {
    ...
    if (domainLower.includes('insurance') || companyLower.includes('insurance')) {
      industry = "Insurance";
      type = "Insurance";
    ...
    } else if (domainLower.includes('myworkdayjobs') || domainLower.includes('oraclecloud')) {
      size = "1000-5000"; // Mid to large enterprise
    } else if (domainLower.includes('fortune') || domainLower.includes('global')) {
      size = "Enterprise"; // Large enterprise
    }
    
    // Location hints from domain patterns
    if (isInsuranceCompany || companyLower.includes('iat') && (domainLower.includes('insurance') || domainLower.includes('iatinsurancegroup'))) {
